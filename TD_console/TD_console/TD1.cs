...
namespace TD_console
{
    public class TD1
    {
        ...
        public static string Inverse_azAZ(string sentence)
        {
            string newSentence = "";
            // Ne rien modifier au dessus de ce commentaire
            foreach(char ret in sentence)
            {
                if (Char.IsUpper(ret)) 
                {
                    newSentence += Char.ToLower(ret);
                }
                else
                {
                    newSentence += Char.ToUpper(ret);
                }
            }
            // Ne rien modifier au dessous de ce commentaire
            return newSentence;
        ...
        public static long River_next(long n)
        {
            long river = 0;
            // Ne rien modifier au dessus de ce commentaire
            string strLong = n.ToString();

            river += n;
            for (int x = 0; x < strLong.Length; x++)
            {
                river += long.Parse("" + strLong[x]);
            }
            // Ne rien modifier au dessous de ce commentaire
            return river;
        }
